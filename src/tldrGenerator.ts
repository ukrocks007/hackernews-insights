...
export async function extractArticleContent(
  ...
  try {
    ...
    const context = await browser.newContext({
      userAgent:
        "Mozilla/5.0 (compatible; HNInsights-TLDR/1.0; +http://example.com)",
      viewport: { width: 1280, height: 720 },
    });

    // Block heavy resources for performance
    await context.route(
      "**/*.{png,jpg,jpeg,gif,webp,svg,woff,woff2,ttf,otf,mp4,mp3,wav,flac,avi,mov,webm}",
      (route) => route.abort(),
    );
    ...
    const paragraphs = await page.$$eval(
      "article p, main p, .article p, .post-content p, .entry-content p",
      (els) => {
        return els
          .map((el) => (el as HTMLElement).innerText.trim())
    ...
    const codeBlocks = await page.$$eval(
      ...
      ({ MIN_TEXT_CHUNK_LENGTH }) => {
        ...
        const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
          acceptNode(node) {
            ...
            if (!text || text.length < MIN_TEXT_CHUNK_LENGTH)
...
export async function generateTLDR(
  ...
  try {
    ...
    if (article.headings.length > 0) {
      contentParts.push(
        `Key sections: ${article.headings.slice(0, 10).join("; ")}`,
      );
    }
...
whether to read the full article.`;
    ...
    logger.info(
      `Generating TLDR for article (content length: ${contentLength} chars)`,
    );
 ...
 */
export async function generateTLDRForURL(
  url: string,
): Promise<TLDRResult | null> {
