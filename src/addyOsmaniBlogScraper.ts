 ...
 */
export async function scrapeAddyOsmaniBlog(
  limit: number = 30,
): Promise<AddyOsmaniBlogItem[]> {
  logger.info(
    `[AddyOsmaniBlog] Starting scrape from ${ADDY_OSMANI_BLOG_URL} (limit=${limit})`,
  );
  ...
  try {
    await page.goto(ADDY_OSMANI_BLOG_URL, {
        ...
        (elements, max) => {
          ...
          for (const el of elements) {
            ...
            if (timeEl) {
              ...
              const datePattern =
                /\b(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d{1,2}\s+\d{4}\b|\d{4}-\d{2}-\d{2}/i;
            ...
            const excerptEl = el.querySelector(
              ".card-description, p, .excerpt, .description, .summary",
            );
            if (
              excerptEl &&
              excerptEl !== titleLink &&
              !excerptEl.contains(titleLink)
            ) {
