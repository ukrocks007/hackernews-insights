...
const BLOCKED_RESOURCE_GLOB =
  "**/*.{png,jpg,jpeg,gif,webp,svg,css,woff,woff2,mp4,mp3}";
...
async function extractContentFromPage(page: Page): Promise<ContentSignals> {
  ...
  const description =
    (await page
      ...
      .catch(() =>
        page.$eval('meta[property="og:description"]', (el) =>
...
function sanitizeDecision(raw: any): BrowsingDecision {
  const allowedActions: Array<BrowsingDecision["action"]> = [
    "click",
    "extract",
    "stop",
  ];
  ...
  const target =
    typeof raw.target === "string" || raw.target === null ? raw.target : null;
  const reason =
    typeof raw.reason === "string" ? raw.reason : "No reason provided";
  return { action, target, reason };
}

async function getBrowsingDecision(
  snapshot: Snapshot,
...
Never propose multiple steps, never include prose outside JSON.`;
  ...
  const decisionTimeout = parsePositiveNumber(
    process.env.FALLBACK_DECISION_TIMEOUT_MS,
    15000,
  );
...
export async function browseWithLLMFallback(
  options: FallbackBrowsingOptions,
  ...
  const navigationTimeout = parsePositiveNumber(
    process.env.FALLBACK_NAV_TIMEOUT_MS,
    20000,
  );
  const fallbackUserAgent =
    process.env.FALLBACK_USER_AGENT || DEFAULT_FALLBACK_USER_AGENT;
  ...
  if (allowlist.length === 0) {
    logger.warn(
      `[fallback][${sourceId}] No valid domain allowlist available. Aborting fallback browsing.`,
    );
    return [];
  }
  ...
  try {
    ...
    while (queue.length > 0) {
      if (Date.now() - startTime > timeoutMs) {
        logger.warn(
          `[fallback][${sourceId}] Global timeout ${timeoutMs}ms reached. Stopping.`,
        );
        break;
      }
      ...
      if (pagesVisited >= maxPages) {
        logger.warn(
          `[fallback][${sourceId}] Max pages (${maxPages}) reached. Stopping.`,
        );
        break;
      }
      ...
      if (visited.has(url)) {
        logger.info(
          `[fallback][${sourceId}] Duplicate URL detected (${url}); skipping.`,
        );
        continue;
      }
      ...
      if (!isDomainAllowed(url, allowlist)) {
        logger.warn(
          `[fallback][${sourceId}] Skipping ${url} (outside allowlist).`,
        );
        continue;
      }
      ...
      try {
        ...
        if (decision.action === "extract") {
          ...
          if (candidates.length >= maxCandidates) {
            logger.info(
              `[fallback][${sourceId}] Reached max candidates (${maxCandidates}). Stopping.`,
            );
            break;
          }
          ...
          if (clicks >= maxClicks) {
            logger.warn(
              `[fallback][${sourceId}] Max clicks (${maxClicks}) hit. Stopping.`,
            );
            break;
          }
          const targetLink = snapshot.links.find(
            (link) => link.id === decision.target,
          );
          if (!targetLink) {
            logger.warn(
              `[fallback][${sourceId}] Invalid click target "${decision.target}".`,
            );
          } else if (!isDomainAllowed(targetLink.href, allowlist)) {
            logger.warn(
              `[fallback][${sourceId}] Blocked navigation to ${targetLink.href} (not in allowlist).`,
            );
          } else if (visited.has(targetLink.href)) {
            logger.warn(
              `[fallback][${sourceId}] Duplicate URL detected (${targetLink.href}); skipping click.`,
            );
          } else if (depth + 1 > maxDepth) {
            logger.warn(
              `[fallback][${sourceId}] Max depth (${maxDepth}) would be exceeded. Stopping navigation.`,
            );
          } else {
          ...
          logger.info(
            `[fallback][${sourceId}] Stopping per model instruction.`,
          );
          break;
        }
      } catch (error) {
        logger.warn(
          `[fallback][${sourceId}] Failed to browse ${url}: ${error}`,
        );
      } finally {
        await page.close();
      }
