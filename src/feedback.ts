...
function calculateSuppressionHours(score: number): number {
  return Math.min(
    ...
    Math.max(
      MIN_SUPPRESSION_HOURS,
      Math.round(Math.abs(score) / SCORE_SCALE) * SUPPRESSION_HOURS_PER_POINT,
    ),
  );
}
...
export function computeRelevanceScore(
  ...
  for (const event of feedbackEvents) {
    ...
    reasons.push(
      `${action} (${event.confidence}) x${decay.toFixed(2)} => ${contribution.toFixed(0)}`,
    );
    ...
    sourceTotals.set(
      event.source,
      (sourceTotals.get(event.source) || 0) + contribution,
    );
  }

  const tagAdjustment = Array.from(tagTotals.values()).reduce(
    (sum, value) => sum + value * TAG_ADJUSTMENT_FACTOR,
    0,
  );
  if (tagAdjustment !== 0) {
  ...
  const sourceAdjustment = Array.from(sourceTotals.values()).reduce(
    (sum, value) => sum + value * SOURCE_ADJUSTMENT_FACTOR,
    0,
  );
  ...
  if (roundedScore < SUPPRESSION_THRESHOLD) {
    ...
    reasons.push(
      `Suppressed for ${hours}h due to low relevance (${toDisplayScore(roundedScore)})`,
    );
  } else if (
    roundedScore > 0 &&
...
export async function recordFeedbackEvent(
...
): Promise<RelevanceComputation | null> {
  ...
  try {
    ...
    const weightMap =
      payload.confidence === "implicit" ? IMPLICIT_WEIGHTS : EXPLICIT_WEIGHTS;
    const topicDelta = weightMap[payload.action] ?? 0;
    if (topicDelta !== 0) {
      const scaled = Math.round(
        topicDelta *
          SCORE_SCALE *
          (payload.confidence === "implicit"
            ? TOPIC_IMPLICIT_SCALE
            : TOPIC_EXPLICIT_SCALE),
      );
      const links = await prisma.storyTopic.findMany({
