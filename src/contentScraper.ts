...
export async function scrapeStoryContent(
  url: string,
  ...
  try {
    ...
    const context = await browser.newContext({
      userAgent:
        "Mozilla/5.0 (compatible; HNInsights/1.0; +http://example.com)",
    ...
    const bodyText = await page.evaluate(
      ({
        ...
        MIN_TRUNCATION_POSITION,
        CONTENT_EXTRACTION_BUFFER_RATIO,
      }) => {
        ...
        const safeCut =
          lastSpace > MIN_TRUNCATION_POSITION ? lastSpace : MAX_CHARS;
    ...
    const hasCodeBlocks =
      (await page.$("pre code, .highlight, .code")) !== null;
